...
public class RTPReceiverThread extends Thread {
	...
	private int numberOfReceivedPackets = 1 ; 
	...
	public void run() {
		...
		while(!rtpSession.endSession) {
			...
			// Prepare a packet
			// the packet size is given by the length of the rtp header + number of byte worth 20ms of audio 
			// i.e 0.02*sampling_rate * bytes_per_sample 
			int myPacketSize = 12 + (int)(0.02*samplingRate*bitsPerSample/8);
			byte[] rawPkt = new byte[myPacketSize];
			...
			if(! rtpSession.mcSession) {
				...
				try {
					if(rtpSession==null){
						System.out.println("RTP SESSION NULL!!!");
					}
					else if (rtpSession.rtpSock == null){
						System.out.println("RTP SOCK NULL");
					}
					rtpSession.rtpSock.receive(packet);
					Log.d("RTP PACKET", "RTP PACKET RECEIVED" + numberOfReceivedPackets) ;
