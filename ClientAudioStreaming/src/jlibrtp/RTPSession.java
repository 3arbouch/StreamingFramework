...
public class RTPSession {
	 ...
	 public int RTPSessionRegister(RTPAppIntf rtpApp, RTCPAppIntf rtcpApp, DebugAppIntf debugApp) {
		if(registered) {
			...
			if (rtpApp instanceof AudioLiveReaderSavingAndPlayer) {

				recvThrd.bitsPerSample=((AudioLiveReaderSavingAndPlayer)rtpApp).getmBitsPerSample();
				recvThrd.samplingRate=((AudioLiveReaderSavingAndPlayer)rtpApp).getmSamplingRate();
			}
	 ...
	 public long[][] sendData(byte[][] buffers, long[] csrcArray, boolean[] markers, long rtpTimestamp, long[] seqNumbers) {
		 ...
		 if(rtpTimestamp < 0)
			 rtpTimestamp += SoundRecorderAndStream.timeStamp;
...
**/
	...
	private void generateCNAME() {
		...
		*/
		cname = System.getProperty("user.name") + "@" + "MohamedBenArbia";
	}
	...
	public RTPReceiverThread getRecvThrd() {
		return recvThrd;
	}
