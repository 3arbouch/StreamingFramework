...
public class RTPSession {
	...
	public Object syncRead = new Object();
	...
	public long[][] sendData(byte[][] buffers, long[] csrcArray,
		...
		for (int i = 0; i < buffers.length; i++) {
			...
			if (markers != null)
				marker = markers[i];
			/**
			 * if(buf.length > 1500) { System.out.println(
			...
			RtpPkt pkt = new RtpPkt(rtpTimestamp, this.ssrc, (int) ret[i][1],
	...
	private void generateCNAME() {
		String hostname;
		/*
		 * if(this.mcSession) { hostname =
		 * this.rtpMCSock.getLocalAddress().getCanonicalHostName(); } else {
